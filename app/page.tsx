...
export default function Home() {
  ...
  // 仅在上传结果从 0 变为非 0 时自动打开侧边栏
  useEffect(() => {
    if (uploadResults.length > 0 && prevResultsLenRef.current === 0) {
      setShowResultSidebar(true)
      setShowPreviewSidebar(false)
    }
    prevResultsLenRef.current = uploadResults.length
  }, [uploadResults])
  ...
  }, [fileDetails, showPreviewSidebar])
    ...
    try {
      ...
      const resultsWithIds = result.results.map(item => {
        ...
        if (item.urls?.original) {
          ...
          if (lastDot > 0) {
            imageId = filename.substring(0, lastDot)
          }
        }
  ...
  const handleDeleteImage = async (id: string) => {
    try {
      ...
      const filename = urlParts[urlParts.length - 1];
      const lastDot = filename.lastIndexOf('.');
      const fileId = lastDot > 0 ? filename.substring(0, lastDot) : filename;
