...
export default function UploadSection({ 
  ...
  const handleFilesSelected = useCallback((files: File[]) => {
    setSelectedFiles(prevFiles => {
      ...
      const newDetails: { id: string; file: File }[] = [];
      ...
      if (newDetails.length === 0) return prevFiles;

      setFileDetails(prevDetails => {
        const updated = [...prevDetails, ...newDetails];
        if (updated.length > maxUploadCount) {
          const allowed = updated.slice(0, maxUploadCount);
          setExceedsLimit(true);
          if (onFilesSelected) onFilesSelected(allowed);
          return allowed;
        }
        setExceedsLimit(false);
      ...
      return newFiles.length > maxUploadCount ? newFiles.slice(0, maxUploadCount) : newFiles;
    });
  }, [maxUploadCount, onFilesSelected])
